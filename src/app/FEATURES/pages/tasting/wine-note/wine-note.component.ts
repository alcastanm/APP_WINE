...
export class WineNoteComponent implements OnInit {
  @ViewChildren('focusable', { read: ElementRef })
  ...
  isLoading = false;
  lottieOk = true;
  imagesRoute = environment.wineImages
  ...
  ngOnInit() {
    if(this.global.note_id != 0)
      {
        this.getNote(this.global.note_id)
      }
    this.initStatusBar();
  }
  ...
  saveWineNote() {
    ...
    formData.append('winenoteid', this.w_noteid.toString());
    ...
    if (this.photo && this.changephoto) {
    ...
    if(this.w_noteid==0)
      {
          this.httpnotes.addNote(formData).subscribe((res: ResultModel) => {
            this.isLoading = false;
            if (res.isSuccess) {
              this.zone.run(() => {
                this.toastService.success(res.returnedMessage);
              });
              this.cleanNote();
            } else {
              this.zone.run(() => {
        ...
        {
          this.httpnotes.updateNote(formData).subscribe((res: ResultModel) => {
            this.isLoading = false;
            if (res.isSuccess) {
              this.zone.run(() => {
                this.toastService.success(res.returnedMessage);
  ...
  {
      this.global.note_id = 0
      let params = {'noteid':note_id}
      this.httpnotes.getnote(params).subscribe
      (
        (res:ResultModel)=>
          {
            if(res.isSuccess)
              {
                  this.w_noteid = res.data.wine_note_id;
                  this.wineName = res.data.wine_name;
                  this.wineType = res.data.wine_type
                  this.colorRating = res.data.color_rating;
                  this.aromaRating = res.data.aroma_rating;
                  this.cuerpoRating = res.data.cuerpo_rating;
                  this.saborRating = res.data.sabor_rating;
                  this.finalRating = res.data.final_rating;
                  this.notes = res.data.notes;
                  this.photo =  `${environment.wineImages}${environment.winesFolder}${res.data.photo}`;
              }else
                {
                  this.toastService.error("Opss!! algo salió mal")
                }
